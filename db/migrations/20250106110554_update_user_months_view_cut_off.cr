class UpdateUserMonthsViewCutOff::V20250106110554 < Avram::Migrator::Migration::V1
  def migrate
    execute <<-SQL
      CREATE OR REPLACE VIEW user_months AS
        WITH months AS (
          ...
          FROM generate_series('2000-01-01'::DATE, CURRENT_DATE + INTERVAL '1 month', '1 month') AS date
        ),
        user_start_month AS (
          SELECT
            users.id AS user_id,
